...
- 🖥 **The PDP-1:** <https://spacewar1962.github.io/spacewar/pdp1.html> · a deep dive into the machine the game was written for, with companion pages on [programming the PDP-1](https://spacewar1962.github.io/spacewar/programming.html) (including an interactive instruction-set lookup), the [Type 30 display](https://spacewar1962.github.io/spacewar/type30.html), and the [PDP-1 as a media machine](https://spacewar1962.github.io/spacewar/music.html) (Peter Samson's Harmony Compiler and Raj Reddy's early speech recognition).
...
| `index.html` + the section pages (`why`, `people`, `timeline`, `ports`, `restoration`, `pdp1`, `programming`, `type30`, `music`, `pdp11`, `minskytron`, `two-cultures`, `play`, `bibliography`, and the port deep-dives `cambridge` / `minnesota` / `stanford` / `mit-ai`) | The project website (served via GitHub Pages), with a grouped dropdown menu. The PDP-1 menu groups the machine, programming (with a live instruction-set lookup, data in `assets/pdp1-instructions.json`), the Type 30 display, and the media-machine page. |
