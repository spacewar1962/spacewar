...
(function (root) {
  ...
  var FINDINGS = [
    ...
    { kind: 'Tape', title: 'Every block checksum on the object tapes checks, except one',
      text: 'Read block by block (Tape view, Anatomy), the surviving object tapes carry 39 to 49 checksummed blocks each after the same 21-word read-in loader, and every checksum agrees with its words except block 2300 to 2400 on sw4.2.bin, off by one. The 2B tape ends without a closing jump to a start address; the dfw 4.1 tape ends with jmp 7751, back into the loader, where the others jump to the program.',
