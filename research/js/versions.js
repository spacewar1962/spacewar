...
(function (root) {
  ...
  var VERSIONS = [
    ...
    { id: '4.1', label: 'Spacewar! 4.1 / 4.2a (dfw)', date: '20 / 22 Feb 1963', sort: 19630220,
      ...
      buildNotes: ["Normalised for assembly: the transcription marks overlined (variable) names with a leading '.' (e.g. '.sx1', '.1sc'); these are read as '~'. Whitespace-only lines (page breaks) are read as blank, so the pt 2 title line is recognised.", "With the June 1963 macro tape as found, the build differs from the authentic dfw tapes in 13 words: ten are 'ioh' (730000 against 760000 on the tapes) and two are the display iot codes in 'dispt i, i my1, 1/2'. The second pair is an assembler matter: the 1963 source writes 'repeat 6<tab>B=B+B' in the dispt macro, which macro1 cuts at the tab; the 1962-63 dialect here reads the whole line, as the tapes show MACRO did.", "With 'ioh' defined as 760000 (opr) in the supplied macro tape, the build is identical to both spacewar4.1_2-20-63_dfw.bin and spacewar4.2a_sa4.bin (2,364 words, no differences). The February 1963 macro system therefore defined ioh differently from the June 1963 tape we hold. SteveRussell_box1/sw4.2.bin differs in one word only (isp 3043 against 3042 at 2333, 'count \\\\src,sq7'). This is a defect, not a different build: the checksum punched after that block (2300 to 2400) on sw4.2.bin is 7262, the same as on spacewar4.1_2-20-63_dfw.bin, but the words on sw4.2.bin sum to one less. The block was punched from 3043, and one hole (the lowest bit) was lost in punching or reading. (Found by the Tape view's anatomy, 26 Sep 2026.)", "The transcription agrees with Russell's punched source tape (see '4.1 / 4.2a (dfw, source tape)') in 1,135 of 1,138 lines; the differences are the scan label '>>32<< 1', a stray '_' at the end, and the tape's overstruck '\u203e+' (\u00b1) in a comment, transcribed as '.+'.", "Punched titles: the leader of SteveRussell_box1/sw4.2.bin reads 'SPACEWAR 4.1 2/20/63 DFW' (frames 44 to 135, letters packed with no gaps), so the tape filed as 4.2 names itself as the 20 February 4.1. spacewar4.1_2-20-63_dfw.bin: after the program's blocks and a jmp 7751 back into the loader come the punched letters 'SPACEWAR SYMZ' (frames 7963 to 8039) and a second part, 12 blocks of 719 words at 6432-7750 ending jmp 1411, which is the symbol table of this assembly: 359 names (three FIO-DEC characters each) with their values, all 235 of the program's symbols and 124 of MACRO's permanent ones (see Findings)."],
    ...
    { id: '4.1t', label: 'Spacewar! 4.1 / 4.2a (dfw, source tape)', date: '20 / 22 Feb 1963', sort: 19630221,
      ...
      buildNotes: ["Read from SteveRussell_box1/spacewar4.1pt1and2.bin, the dfw source punched in FIO-DEC: 15,389 frames, every one passing the odd-parity check, 26 stop codes (page breaks).", "Assembled with the June 1963 macro tape patched to 'ioh=760000', the tape rebuilds spacewar4.1_2-20-63_dfw.bin and spacewar4.2a_sa4.bin exactly: an authentic source reproducing authentic binaries. sw4.2.bin differs in one word, which its own block checksum shows to be a lost hole: the same build.", "Overlined variables are punched with the non-spacing overbar (lower-case 056), read as '\\\\'; the one overstrike, '\u203e+' (\u00b1), is in a comment.", "Punched titles: the leader of SteveRussell_box1/sw4.2.bin reads 'SPACEWAR 4.1 2/20/63 DFW' (frames 44 to 135, letters packed with no gaps), so the tape filed as 4.2 names itself as the 20 February 4.1. spacewar4.1_2-20-63_dfw.bin: after the program's blocks and a jmp 7751 back into the loader come the punched letters 'SPACEWAR SYMZ' (frames 7963 to 8039) and a second part, 12 blocks of 719 words at 6432-7750 ending jmp 1411, which is the symbol table of this assembly: 359 names (three FIO-DEC characters each) with their values, all 235 of the program's symbols and 124 of MACRO's permanent ones (see Findings)."],
